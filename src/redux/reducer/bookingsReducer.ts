import { Booking } from "../actions/action-creators";
...
const initialState: BookingsStoreState = {
    bookings: []
}

export const bookingsReducer = (state = initialState, action: Action) => {
    switch(action.type) {
        case ActionTypes.fetchBookings:
            return {
                ...state,
                bookings: action.payload
            }

        case ActionTypes.deleteBooking:
            return {
                ...state,
                bookings: state.bookings.filter((b: Booking) => b.id !== action.payload)
            }
    }
}
